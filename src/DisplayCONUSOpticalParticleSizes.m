...
global GOESFileName iaddLightning;
...
global GMTCPSFracS GMTCPSFracE;
...
% Get the scan start date and time data based on the file name
GMTCPSIS=60*MinuteS+SecondS;
GMTCPSFracS=HourS+GMTCPSIS/3600;
GMTCPSIE=60*MinuteE+SecondE;
GMTCPSFracE=HourE+GMTCPSIE/3600;
...
if(iaddLightning==1)
    ...
    while(igo2>0)
        ...
        if(itype<3)
            if((TS>=GMTCPSFracS) && (TE<=GMTCPSFracE))
                igo2=0;
